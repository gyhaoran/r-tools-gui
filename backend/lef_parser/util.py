...
def draw_obs(obs, color, ax):
    """
    Helper method to draw a OBS object on a specified Axes.
    :param obs: OBS object
    :param color: color of the OBS
    ...
    for layer in obs.info["LAYER"]:
        for shape in layer.shapes:
            ...
            ax.add_patch(draw_shape)
            

def draw_port(port, color, ax, fill=True):
    """
    Helper method to draw a PORT object on a specified Axes.
    ...
    for layer in port.info["LAYER"]:
        for shape in layer.shapes:
            ...
            if shape.type == "RECT":
                scaled_pts = rect_to_polygon(scaled_pts)
            draw_shape = plt.Polygon(scaled_pts, closed=True, fill=fill, color=color)
            ax.add_patch(draw_shape)


def draw_pin(pin, ax):
    """
    Helper method to draw a PIN object on a specified Axes.
    :param pin: a pin object
    :param ax: a Matplotlib Axes instance
    :return: void
    """
    color = "blue" if pin.name.lower() in ["vdd", "gnd", "vss"] else "red"
    draw_port(pin.info["PORT"], color, ax, False)
...
def draw_macro(macro, ax):
    """
    Function to draw a Macro (cell) object on a specified Axes.
    ...
    if "OBS" in macro.info:
        draw_obs(macro.info["OBS"], "blue", ax)
    # draw each PIN
